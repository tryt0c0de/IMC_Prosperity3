...
class Trader:
    ...
    def market_make(
        ...
        scaling_factor = max(0.2, 1 - abs(position_ratio))  # Scale from 100% down to 20%

        buy_quantity = int((position_limit - position - buy_order_volume) * scaling_factor)
        if buy_quantity > 0:
            orders.append(Order(product, round(bid), buy_quantity))  # Buy order

        sell_quantity = int((position_limit + position - sell_order_volume) * scaling_factor)
        if sell_quantity > 0:
