...
def test_single_day(rolling_window, day_number):
    command = ["prosperity3bt", r"C:\Users\Usuario1\Desktop\quant\IMC_Prosperity\Round1\Miquel\prev_year_trader.py", f"1--{day_number}", "--parameters", f"{rolling_window}"]
...
def test_full_round(limits):
    command = ["prosperity3bt", r"C:\Users\Usuario1\Desktop\quant\IMC_Prosperity\Round1\Miquel\prev_year_trader.py", "1", "--parameters", f"{limits}"]
    ...
    if match:
        ...
        if total_profit_match:
            ...
            # Rename the log file to include the parameters and the profit in a custom folder
            custom_log_path = log_path.replace(log_path.split('/')[-1], f"Round1/Miquel/backtests/miquel_tester_full_round_limit_rainforest{limits[0]}_kelp{limits[1]}_pnl_{total_profit}.log")
            print(custom_log_path)
            os.rename(log_path, custom_log_path)
...
limits_kelp= [i for i in range(0,50)]
for i in limits_kelp:
    if test_type == "single_day":
        total_profit = test_single_day([20,i], day_to_test)
    else:  # full_round
        total_profit = test_full_round([20,i])
