...
def test_single_day(rolling_window, day_number):
    command = ["prosperity3bt", r"/Users/maximesolere/Library/Mobile Documents/com~apple~CloudDocs/Maxime/RIBOT/Git/Tutorial/Max/Trader.py",
               f"1--{day_number}", "--parameters", f"{rolling_window}"]
    ...
    match = re.search(r'Tutorial/Maxbacktests[\\/][\d\-_.]+\.log', full_output)
    if match:
        ...
        if profit_match:
            ...
            custom_log_path = log_path.replace(log_path.split('/')[-1],
                                               f"Tutorial/Max/backtests/miquel_tester_day_{day_number}_rolling_window_{rolling_window}_pnl_{total_profit}.log")
